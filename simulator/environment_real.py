...
TODO: build config files which have estimated test time details in the products in place of configurations
K to P
TODO: Functions to modify for real time inference and maybe training
TODO : Test cases for environment simulation and schedule formation
And all the inherited functions
compute_remaining_setup_plus_test_time_of_job
compute_estimated_operation_test_time_under_configuration
compute_remaining_setup_plus_test_time_of_job
compute_global_remaining_test_time_of_jobs
compute_opspecific_remaining_test_time_of_job
compute_local_bag_remaining_test_time_of_jobs
...
class SchedulingEnv(MultiAgentEnv):
    ...
    def __init__(self, config=None):
        ...
        self.differences = [random.uniform(1, 10)]
        # Agents
        ...
        for testerName in self.data['testers']['items'].keys():
            self.observation_space[OS_AGENT_PREFIX + testerName] = gym.spaces.Box(low=np.float32(-np.inf),
                                                                                  high=np.float32(np.inf), shape=(26,),
                                                                                  dtype=np.float32)
    ...
    def step(self, actions: MultiAgentDict) -> Tuple[
        ...
        for agentName, action in actions.items():
            if agentName == MS_AGENT:
                ...
                print('MS Agent Output', testerName)
            else:
        ...
        for agentName in actions.keys():
            if agentName == MS_AGENT:
                # change this for experimentation
                rewards[agentName] = self._calculate_msagent_fuzzy_reward(testerName)
            else:
                rewards[agentName] = self._calculate_osagent_minmax_reward(agentName, self.differences)
        ...
        for testerName, testerDetails in self.testers.items():
            if testerDetails['testerStatus'] == TesterStatus.BUSY:
                ...
                print('current operation', currentOp)
                names = currentOp.split(HYPHEN)
                jobName = names[0] + HYPHEN + names[1]+ HYPHEN + names[2]
                ...
                if self.operations[currentOp]['completionTime'] <= self.time:
                    ...
                    self.jobs[jobName][
                        'estimatedRemainingTime'] = obs_utils.compute_remaining_setup_plus_test_time_of_job(self.jobs,
                                                                                                            self.operations,
                                                                                                            jobName,
                                                                                                            self.time)
                    ...
                    for childOpNum in self.operations[currentOp]['childOperations']:
                        print('child operation', childOpNum)
                        childOpName = names[0] + HYPHEN + names[1] + HYPHEN + names[2] + HYPHEN + str(childOpNum)
    ...
    def render(self) -> None:
        ...
        for event in self.history[self.historyIdx:]:
            ...
            configIdx = self.data['configurations']['items'][self.operations[event['opId']]['configurationUsed']][
                'index']
            productIdx = self.data['products']['items'][self.operations[event['opId']]['productName']]['index']

            plt.barh(y=event['testerName'], width=setupTime, left=event['time'], height=0.4,
                     color=setupCmap(setupNorm(configIdx)))
            plt.barh(y=event['testerName'], width=testTime, left=event['time'] + setupTime, height=0.2,
                     color=testCmap(testNorm(productIdx)))
            plt.barh(y=event['testerName'], width=0.2, left=event['time'] + setupTime + testTime, height=0.4,
                     color='black')

            center_x = event['time'] + setupTime
            center_y = event['testerName']
            plt.text(x=center_x, y=center_y, s=event['opId'], ha='left', va='bottom', rotation='horizontal',
        ...
        if self.setupRendering == True and self.closeEnv == True:
            plt.savefig(self.staticConfigFileName.split('.')[0] + (
                '_greedy' if self.followGreedyStrategy else '_`model') + '.png')
            plt.close()
    ...
    def _find_best_possible_time_jump(self) -> float:
        ...
        for testerName, testerDetails in self.testers.items():
            if testerDetails['testerStatus'] == TesterStatus.IDLE and self.localBags[testerName].size() > 0:
                return self.time, TimeJumpReason.NO_JUMP
            if testerDetails['testerStatus'] == TesterStatus.BUSY:
                jump_times.append((self.operations[testerDetails['currentOperationName']]['completionTime'],
                                   TimeJumpReason.TEST_DONE))
    ...
    def _calculate_msagent_reward(self, testerName) -> float:
        ...
        a = reward_utils.compute_msagent_reward_total_setups_difference_at_tester(self.data, self.jobs, self.operations,
                                                                                  self.localBags,
                                                                                  self.prevStateAttributes, testerName)
        ...
        b = 0
        c = -reward_utils.compute_msagent_reward_skewness_difference_at_tester(self.data, self.operations,
                                                                               self.localBags[testerName],
                                                                               self.prevStateAttributes, testerName)
        d = -reward_utils.compute_msagent_reward_kurtosis_difference_at_tester(self.data, self.operations,
                                                                               self.localBags[testerName],
                                                                               self.prevStateAttributes, testerName)

        if math.isnan(a) or math.isnan(b) or math.isnan(c) or math.isnan(d) or a == float('inf') or b == float(
                'inf') or c == float('inf') or d == float('inf') or a == float('-inf') or b == float(
            '-inf') or c == float('-inf') or d == float('-inf'):
    ...
    #     return a
    def _calculate_msagent_fuzzy_reward(self, testerName) -> float:
        """Returns the reward for MSAgent. Calculated as below:

        Reward is the hammacher product of exponential of below components
        - a. At this tester, (Number of setups before - Number of setups after this action)
        - c. At this tester, difference of skewness of duedates before and after applying this action
        - d. At this tester, difference of kurtosis of duedates before and after applying this action
        """
        ...
        a = reward_utils.compute_msagent_reward_total_setups_difference_at_tester(self.data, self.jobs, self.operations,
                                                                                  self.localBags,
                                                                                  self.prevStateAttributes, testerName)
        # b = reward_utils.compute_msagent_reward_difference_in_max_duedate_and_completion_time(self.data,
        # self.operations, self.configurations, self.localBags, testerName)
        b = 0
        c = -reward_utils.compute_msagent_reward_skewness_difference_at_tester(self.data, self.operations,
                                                                               self.localBags[testerName],
                                                                               self.prevStateAttributes, testerName)
        ...
        if math.isnan(a) or math.isnan(b) or math.isnan(c) or math.isnan(d) or a == float('inf') or b == float(
            ...
            raise Exception('MSAgent Reward invalid value: ', a, b, c, d)
        reward = [a, d, c]
        return reward_utils.compute_sigmoid(sum(reward))

    def _calculate_msagent_hamacher_reward(self, testerName) -> float:
        """Returns the reward for MSAgent. Calculated as below:
        ...
        a = reward_utils.compute_msagent_reward_total_setups_difference_at_tester(self.data, self.operations,
                                                                                  self.localBags,
                                                                                  self.prevStateAttributes, testerName)
        # b = reward_utils.compute_msagent_reward_difference_in_max_duedate_and_completion_time(self.data,
        # self.operations, self.configurations, self.localBags, testerName)
        ...
        return reward_utils.hamacher_T_norm(a, reward_utils.hamacher_T_norm(c, d))

    def _calculate_osagent_reward(self, agentName, differences) -> float:
        """Returns the reward for OSAgent. Calculated as below:
           Add priority factor to the reward in the end
        - | Difference of tardiness before and after state update for the corresponding tester |
        """
        testerName = agentName[len(OS_AGENT_PREFIX):]

        # a = reward_utils.compute_osagent_reward_tardiness_difference(self.data, self.jobs, self.operations,
        # self.testers, self.configurations, self.prevStateAttributes, testerName, self.time)

        a, min, max = reward_utils.compute_osagent_reward_tardiness_difference(self.data, self.jobs, self.operations,
                                                                               self.testers,
                                                                               self.configurations,
                                                                               self.prevStateAttributes,
                                                                               testerName, self.time, differences)
        if math.isnan(a) or a == float('inf') or a == float('-inf'):
            raise Exception('OSAgent Reward invalid value: ', a)
        a = reward_utils.compute_sigmoid(a)
        return a

    def _calculate_osagent_minmax_reward(self, agentName, differences) -> float:
        """Returns the reward for OSAgent. Calculated as below:
           Add priority factor to the reward in the end
        - | Difference of tardiness before and after state update for the corresponding tester |
        """
        ...
        # a = reward_utils.compute_osagent_reward_tardiness_difference(self.data, self.jobs, self.operations,
        # self.testers, self.configurations, self.prevStateAttributes, testerName, self.time)
        # print(self.operations['P199223J-01L-0-0'])
        a, min, max = reward_utils.compute_osagent_reward_tardiness_difference(self.data, self.jobs, self.operations,
                                                                               self.testers,
                                                                               self.configurations,
                                                                               self.prevStateAttributes,
        ...
        return reward_utils.compute_minmax(a, min, max)
    ...
    def _handle_msagent_action(self, action) -> None:
        ...
        if self.followGreedyStrategy == True:
            testerName = action_utils.select_msagent_tester_for_given_operation_following_greedy_strategy(self.data,
                                                                                                          self.jobs,
                                                                                                          self.operations,
                                                                                                          self.testers,
                                                                                                          self.configurations,
                                                                                                          self.localBags,
                                                                                                          operation,
                                                                                                          self.msGreedyStrategy)
        else:
            testerName = action_utils.select_msagent_tester_for_given_operation(self.data, self.jobs, self.operations,
                                                                                self.testers, self.configurations,
                                                                                self.localBags, operation, action)
    ...
    def _handle_osagent_action(self, agentName, action) -> None:
        ...
        if self.followGreedyStrategy == True:
            operation = action_utils.select_osagent_operation_for_given_tester_following_greedy_strategy(self.data,
                                                                                                         self.jobs,
                                                                                                         self.operations,
                                                                                                         self.testers,
                                                                                                         self.configurations,
                                                                                                         self.localBags,
                                                                                                         testerName,
                                                                                                         self.osGreedyStrategy,
                                                                                                         self.time)
        else:
            operation = action_utils.select_osagent_operation_for_given_tester(self.data, self.jobs, self.operations,
                                                                               self.testers, self.configurations,
                                                                               self.localBags, testerName, action,
                                                                               self.time)
        ...
        ## TODO change the best config extraction and make it per product
        best_config = obs_utils.compute_best_config_for_operation_on_a_tester_per_product(self.data, self.jobs, self.operations, operation,
                                                                              testerName)
        ...
        if self.operations[operation]['isRootOp'] == True:
            ...
            ## and access the test time using sampling in obs utils
            currProduct = self.jobs[self.operations[operation]['jobName']]['productName']
        ...
        self.operations[operation]['setupTime'] = 0 if prevConfig == currConfig else \
            self.data['configurations']['items'][currConfig]['setupTimes'][
                self.data['configurations']['items'][prevConfig]['index']]
        # print(operation)

        # change this to take the estimated test time from a product rather than a configuration
        self.operations[operation]['estimatedTestTime'] = obs_utils.compute_estimated_operation_test_time_under_product(self.data,
                                                                                                       self.operations[
                                                                                                           operation][
                                                                                                           'opType'],
        ...
        self.operations[operation]['completionTime'] = self.time + self.operations[operation]['setupTime'] + \
                                                       self.operations[operation]['estimatedTestTime']
    ...
    def _build_observations_dict(self) -> dict:
        ...
        if self.globalBag.size() > 0:
            ...
            for testerName, testerDetails in self.testers.items():
                if testerDetails['testerStatus'] == TesterStatus.IDLE and self.localBags[testerName].size() > 0:
                    observations[OS_AGENT_PREFIX + testerName] = self._build_osagent_observation(
                        self.localBags[testerName], testerName)
    ...
    def _init_internal_state(self, data):
        ...
        self.configurations = {}
        ## configuration independent items are still the same no need to change
        ...
        for productName, productDetails in data['products']['items'].items():
            ...
            for opNum, opType in enumerate(data['products']['items'][productName]['operations']):
                ...
                in_degree[opNum] = 0
                out_degree[opNum] = 0
                avg_setup_time[opNum] = obs_utils.compute_avg_operation_setup_time_per_product(self.data, self.configurations, opType)
                avg_test_time[opNum] = obs_utils.compute_avg_operation_test_time_per_product(self.data, opType)
            ...
            # print(roots)
            for rootOp in roots:
                self._find_op_due_date(operation_adjacency_list, productName, rootOp, op_due_dates, avg_setup_time,
                                       avg_test_time)
            """
            for op in range(len(data['products']['items'][productName]['operations'])):
        ...
        for testerName in data['testers']['items'].keys():
            ...
            # large number because initialising with 0 means negative reward
            self.prevStateAttributes['tardiness'][testerName] = float(random.randint(100, 1000))
        ...
        if op_due_dates[root] == None:
            if len(adjacencyList[root]) == 0:
                ...
                for child in adjacencyList[root]:
                    min_due_date = min(min_due_date,
                                       self._find_op_due_date(adjacencyList, productName, child, op_due_dates,
                                                              avg_setup_time, avg_test_time))
    ...
    def _build_msagent_global_observation_list(self) -> list:
        ...
        return obs_utils.flatten([
            ...
            ## TODO change this extractions
            obs_utils.compute_global_number_of_compatible_configurations_per_product(self.data, self.operations, self.jobs, self.globalBag),
            obs_utils.compute_global_due_dates(self.operations, self.globalBag),
            obs_utils.compute_global_remaining_test_time_of_jobs(self.data, self.jobs, self.operations, self.globalBag,
                                                                 self.time),
    ...
    def _build_msagent_opspecific_observation_list(self, opName) -> list:
        ...
        """
        ## TODO change the configurations extractions here
        return [
            obs_utils.compute_opspecific_avg_setup_plus_test_time_per_product(self.data, self.jobs, self.operations, opName),
            obs_utils.compute_opspecific_no_of_compatible_configurations_per_product(self.data, self.jobs, self.operations, opName),
    ...
    def _build_msagent_local_observation_list(self) -> list:
        ...
        return obs_utils.flatten([
            ...
            obs_utils.compute_local_total_setup_plus_test_times(self.data, self.jobs, self.operations, self.configurations,
                                                                self.localBags),
            obs_utils.compute_local_number_of_setups(self.data, self.jobs, self.operations, self.localBags),
            obs_utils.compute_local_duedate_skewness(self.data, self.operations, self.localBags),
            obs_utils.compute_local_duedate_kurtosis(self.data, self.operations, self.localBags),
            obs_utils.compute_local_ratio_of_setup_to_test_time_per_product(self.data, self.operations, self.jobs, self.configurations,
                                                                self.localBags)
        ])
    ...
    def _build_osagent_observation(self, localBag, testerName) -> list:
        ...
        return obs_utils.flatten([
            ...
            obs_utils.compute_local_bag_total_setup_plus_test_time(self.data, self.jobs, self.operations, self.configurations,
                                                                   localBag, testerName),
