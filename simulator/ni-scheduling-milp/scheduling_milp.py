...
class SchedulingMILP:
    ...
    def _load_model(self):
        ...
        """
        model_proto = None
        with open(DATA_ROOT + "_".join([str(item) for item in [self.startTime, self.scName, self.relativeMIPGap, self.timeLimit, self.concurrentThreads, self.solverName]]) + '.txt', "rb") as model_file:
                ...
                for opNum, opType in enumerate(productDetails['operations']):
                    ...
                    self.operations[opName] = {
                        ...
                        'testTime': { # TODO: instead of mean, can sample from the distribution
                            configName: data['operations']['items'][opType]['estimatedTestTime'][configName]['mean'] for configName in compatibleConfigurations
                        }
                    }
    ...
    def _add_variables(self):
        ...
        for opName, opDetails in self.operations.items():
            ...
            for testerName, testerDetails in self.testers.items():
                self.solver.BoolVar(f'ybar_{opName}_{testerName}')          # Ybar ybar_jo_m - True if jo is the first test on tester m
                self.solver.BoolVar(f'ycap_{opName}_{testerName}')          # Ycap ycap_jo_m - True if jo is the last test on tester m
                for configurationName in testerDetails['compatibleConfigurations']:
                    self.solver.BoolVar(f'z_{opName}_{testerName}_{configurationName}')      # Z z_jo_mk      - True if jo is tested on mk
                for anotherOpName, anotherOpDetails in self.operations.items():
                    if anotherOpName != opName:
                        self.solver.BoolVar(f'y_{opName}_{anotherOpName}_{testerName}')      # Y_jo_jdashodash_m - True if jdashodash follows jo o tester m
    ...
    def _add_constraints(self):
        ...
        for testerName, testerDetails in self.testers.items():
            for opName, opDetails in self.operations.items():
                for anotherOpName, anotherOpDetails in self.operations.items():
                    if anotherOpName != opName:
                        self.solver.Add(self._var(f's_{opName}') >= self._var(f's_{anotherOpName}') + self._var(f'PT_{anotherOpName}') + self._var(f'RT_{opName}') + self.G * (self._var(f'y_{anotherOpName}_{opName}_{testerName}') - 1), f'Constraint_2_{testerName}_{opName}_{anotherOpName}')

        # New Constraint 13: Start time of the first operation is after its reconfiguration is done (edge-case for Constraint #2)
        for testerName, testerDetails in self.testers.items():
            for opName, opDetails in self.operations.items():
                self.solver.Add(self._var(f's_{opName}') >= self._var(f'RT_{opName}') + self.G * (self._var(f'ybar_{opName}_{testerName}') - 1), f'Constraint_13_{testerName}_{opName}')
        ...
        for jobName, jobDetails in self.jobs.items():
            for opName in jobDetails['operations']:
                for parentOpNum in self.operations[opName]['parentOperations']:
                    ...
                    self.solver.Add(self._var(f's_{opName}') >= self._var(f's_{parentOpName}') + self._var(f'PT_{parentOpName}'), f'Constraint_9_{opName}_{parentOpName}')
                ...
                for configurationName in possibleConfigurations:
                    ...
                    for anotherOpName, anotherOpDetails in self.operations.items():
                        if (opName != anotherOpName):
                            ...
                            anotherPossibleConfigurations = set(anotherOperationConfigurations) & set(testerConfigurations)
                            for anotherConfigurationName in anotherPossibleConfigurations:
                                ...
                                self.solver.Add(self._var(f'RT_{opName}') >= setupTime + self.G * (self._var(f'z_{opName}_{testerName}_{configurationName}') + self._var(f'z_{anotherOpName}_{testerName}_{anotherConfigurationName}') + self._var(f'y_{anotherOpName}_{opName}_{testerName}') - 3), f'Constraint_10_{testerName}_{opName}_{configurationName}_{anotherOpName}_{anotherConfigurationName}')
        ...
        for testerName, testerDetails in self.testers.items():
            for opName, opDetails in self.operations.items():
                ...
                for configurationName in possibleConfigurations:
                    ...
                    self.solver.Add(self._var(f'RT_{opName}') >= setupTime + self.G * (self._var(f'z_{opName}_{testerName}_{configurationName}') + self._var(f'ybar_{opName}_{testerName}') - 2), f'Constraint_11_{testerName}_{opName}_{configurationName}')
        ...
        for testerName, testerDetails in self.testers.items():
            for opName, opDetails in self.operations.items():
                ...
                for configurationName in possibleConfigurations:
                    ...
                    self.solver.Add(self._var(f'PT_{opName}') >= testTime + self.G * (self._var(f'z_{opName}_{testerName}_{configurationName}') - 1), f'Constraint_12_{testerName}_{opName}_{configurationName}')
        ...
        for testerName, testerDetails in self.testers.items():
            ...
            for opName, opDetails in self.operations.items():
                ...
                for configurationName in possibleConfigurations:
                    zsum += self._var(f'z_{opName}_{testerName}_{configurationName}')
            for opName, opDetails in self.operations.items():
                self.solver.Add(self.G * (2 - self._var(f'ybar_{opName}_{testerName}') - self._var(f'ycap_{opName}_{testerName}')) >= (zsum - 1), f'Constraint_14_{testerName}_{opName}')
        ...
        for jobName, jobDetails in self.jobs.items():
            self.solver.Add(self._var(f'T_{jobName}') >= self._var(f'C_{jobName}') - jobDetails['duedate'], f'Constraint_16_{jobName}')

        ## Constraint 17: Completion time C_j defining constraint
        for jobName, jobDetails in self.jobs.items():
            for opName in jobDetails['operations']:
                self.solver.Add(self._var(f'C_{jobName}') >= self._var(f's_{opName}') + self._var(f'PT_{opName}'), f'Constraint_17_{opName}')
    ...
    def _plot_and_save(self):
        ...
        for opName, opDetails in self.operations.items():
            ...
            ax.barh(y=testerId, width=pt, left=s, height=0.5, color=jobColors[self.jobs[self.operations[opName]['jobName']]['uniqueJobId']])
            center_x = s
        ...
        # Save the plot
        plt.savefig(DATA_ROOT + "_".join([str(item) for item in [self.startTime, self.scName, self.relativeMIPGap, self.timeLimit, self.concurrentThreads, self.solverName]]) + '.png')
    ...
    def _print_op_wise_results(self):
        for opName, opDetails in self.operations.items():
            print(f"Operation: ({opName})", f"Tester: {self._find_tester_scheduled(opName)}", f"Start: {self._var(f's_{opName}').solution_value()}", f"RT: {self._var(f'RT_{opName}').solution_value()}", f"PT: {self._var(f'PT_{opName}').solution_value()}")
